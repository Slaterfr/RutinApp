...
from ..models.models import Ejercicio, DiaRutina, detalleEjercicio
...
class ExerciseService:
    def __init__(self):
        self.crud_exercise = CRUDBase(Ejercicio)
        self.crud_details = CRUDBase(detalleEjercicio)
    ...
    def get_all(self, limit: int = 6, category: str = ""):
        ...
        with database.session as sess:
            query = sqlm.select(Ejercicio)
            if category:
                query = query.where(Ejercicio.category.contains(category))
    ...
    def add_detail(self, exercise_id: int, data):
        ...
        with database.session as sess:
            exercise = sess.exec(sqlm.select(Ejercicio).where(Ejercicio.id == exercise_id)).first()
            if not exercise:
    ...
    def get_by_day(self, day_id: int):
        ...
        with database.session as sess:
            day = sess.exec(sqlm.select(DiaRutina).where(DiaRutina.id == day_id)).first()
            if not day:
